...
/* ── State ── */
let scene, camera, renderer, controls, globeMesh, signalDots = [];
let audioCtx, masterGain, audioOn = false;
let userLat = 33.57, userLng = -117.73, userCity = 'Aliso Viejo', userCountry = 'USA';
...
window.toggleAudio = function() {
    if (!audioOn) {
        ...
        localGain.gain.setValueAtTime(0.05, audioCtx.currentTime);
...
/* ── Globe (High-Fidelity "Google Earth" Style) ── */
function initGlobe() {
    const canvas = document.getElementById('globe-canvas');
    scene = new THREE.Scene();
    camera = new THREE.PerspectiveCamera(45, window.innerWidth / window.innerHeight, 0.1, 1000);
    ...
    const loader = new THREE.TextureLoader();
    // High-res imagery for professional aesthetic
    const dayTexture = loader.load('https://unpkg.com/three-globe/example/img/earth-blue-marble.jpg');
    const bumpMap = loader.load('https://unpkg.com/three-globe/example/img/earth-topology.png');
...
window.onMoodInput = function(val) {
    // Optional: Real-time UI feedback here
};
...
window.submitMood = function() {
    ...
    // Populate and show persistent Aura Card
    document.getElementById('aura-word').textContent = word.toUpperCase();
    document.getElementById('aura-loc').textContent = `${userCity}, ${userCountry}`;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Silent GeoDetect with Aliso Viejo fallback
    fetch('https://ipapi.co/json/').then(res => res.json()).then(data => {
        userCity = data.city || userCity;
        userCountry = data.country_name || userCountry;
        userLat = data.latitude || userLat;
        userLng = data.longitude || userLng;
