...
function startDrone(mood){
    if(!actx) return;
    drones.forEach(o => { try{o.stop()}catch(e){} }); drones = [];
...
window.submitMood = async () => {
    const inp = document.getElementById('mood-in'); const word = inp.value.trim(); if(!word) return;
    const mood = word.length % 2 === 0 ? 'joy' : 'hope'; curMood = mood;
    ...
    inp.value = ''; updateBars();
};

window.shareMood = () => {
    html2canvas(document.getElementById('capture-area')).then(canvas => {
        const link = document.createElement('a'); link.download = `Aura-${userCity}.png`;
...
function addFeed() {
    ...
    fi.innerHTML = `<div style="font-family:'Bebas Neue'; font-size:14px;">${m.toUpperCase()}</div><div style="font-size:8px; opacity:0.5">${userCity}</div>`;
    list.insertBefore(fi, list.firstChild); if(list.children.length > 4) list.removeChild(list.lastChild);
...
function updateBars() {
    ...
    [{l:'CALM',p:45},{l:'JOY',p:30},{l:'HOPE',p:25}].forEach(s => {
        ...
        r.innerHTML = `<div style="display:flex; justify-content:space-between; font-size:9px; margin-bottom:3px"><span>${s.l}</span><span>${s.p}%</span></div>
                       <div style="height:1px; background:rgba(255,255,255,0.1); width:100%"><div style="height:100%; background:var(--mc); width:${s.p}%"></div></div>`;
        b.appendChild(r);
    });
}

// MAP & INTERACTION
window.zoomBy = (f) => { sc = Math.max(0.5, Math.min(20, sc * f)); drawAll(); };
...
function drawAll(){
    ...
    const marsala = {n:'Marsala', lat:37.80, lon:12.44}; const [px,py] = proj(marsala.lon, marsala.lat);
    ctxC.beginPath(); ctxC.arc(px,py,5,0,Math.PI*2); ctxC.fillStyle='gold'; ctxC.fill();
    const d = Math.hypot(px - mX, py - mY);
    const tip = document.getElementById('tip');
    if(d < 10) { tip.style.left = (mX+15)+'px'; tip.style.top = (mY+15)+'px'; tip.style.opacity = 1;
...
function drawMap(){
    if(!geoFeatures.length) return; ctxM.clearRect(0,0,W,H); ctxM.strokeStyle='rgba(93,220,184,0.3)'; ctxM.lineWidth=1/sc;
